...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#ifndef  RT_HW_ETHERNET_h
#define  RT_HW_ETHERNET_h
//-------------------------------------------------------------------------------------------------
#include "RT_HW_BASE.h"									//--Подключение базовой библиотеки;
#include "flprogEthernet.h"           					//--Подключение библиотеки Ethernet FLProg; 
#define   RT_HW_ETHERNET_VER 		 (uint8_t)102		//--Номер версии;
#define   RT_HW_ETHERNET_VER_NAME 	"1.0.2 01.01.2024"	//--Наименование версии;
//=================================================================================================
//                            1.КЛАСС расширения  "flprogEthernet.h" (возможно уйдет в flprogEthernet.h);
//  statusError: 
//		bit.15 недопустимый номер шины;
//		bit.14 недопустимый pin СЫ;
// status (flprogEthernet.h):
// FLPROG_NOT_REDY_STATUS 						0
// FLPROG_READY_STATUS 							1
// FLPROG_WAIT_ETHERNET_HARDWARE_INIT_STATUS 	2
// FLPROG_WAIT_ETHERNET_DHCP_STATUS 			3
// FLPROG_WAIT_ETHERNET_DNS_STATUS 				4
// FLPROG_WAIT_ETHERNET_UDP_STATUS 				5
// FLPROG_WAIT_ETHERNET_CLIENT_CONNECT_STATUS 	6
// FLPROG_WAIT_ETHERNET_LINK_ON_STATUS 			7
// FLPROG_ETHERNET_HARDWARE_INIT_STATUS 		8
// FLPROG_WAIT_ETHERNET_CONNECT_STATUS 			9
// FLPROG_WAIT_WIFI_CLIENT_CONNECT_STATUS 		10
// FLPROG_WAIT_ETHERNET_START_CLIENT_CONNECT_STATUS 11
// FLPROG_WAIT_WEB_SERVER_READ_REQEST 			12
// FLPROG_WAIT_WEB_SERVER_SEND_ANSVER 			13
// FLPROG_WAIT_SEND_UDP_PACAGE 					14
//
// =0  Нет готовности;
// =1  Ethernet готов;
// =2  Ожидание инициализация устройства;
// =3  Ожидание DHCP
// =4  Ожидание DNS
// =5  Ожидание UDP
// =6  Ожидание соединения с клиентом через Ethernet;
// =7  Ожидание подтверждения соединения
// =8  Инициализация оборудования;
// =9  Ожидание соединения;
// =10 Ожидание соединения с клиентом через Wi-Fi; 
// =11 Ожидание начала соединения с клиентом;
// =12 Ожидание ответ  от Web-сервера;
// =13 Ожидание запроса на Web-сервер;
// =14 Ожидание отправки пакета на UDP-сервер;
//=================================================================================================
...
uint8_t  adrMAC[6]={0x78, 0xAC, 0xC0, 0x2C, 0x3E, 0x44};
...
if(dir==0){
  ...
  id.mac(RT_HW_Base.buff.mac[0],RT_HW_Base.buff.mac[1],RT_HW_Base.buff.mac[2],		//--Установка MAC-адреса
         RT_HW_Base.buff.mac[3],RT_HW_Base.buff.mac[4],RT_HW_Base.buff.mac[5]);		//   из буфера RT_HW_Base (генерится случайное значение); 	
...
if(freshConnect){freshConnect=0;  
  ...
  //RT_HW_Base.consoleTest(String(F(" chip")),            id.getChip(),';','U'); //--Функция не реализована(ошибка компиляции);
   RT_HW_Base.consoleCR();  
  RT_HW_Base.consoleBlank(13); RT_HW_Base.consoleTestArr(String(F("MAC")),RT_HW_Base.buff.mac,'~','H',6); 
  RT_HW_Base.consoleBlank(11);  Serial.print("IP=");      Serial.print(id.localIP());
  RT_HW_Base.consoleCR();
  RT_HW_Base.consoleBlank(13); 
