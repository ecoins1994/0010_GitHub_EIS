...
if(!id.available())       {return;}
  if(mode>=1){RT_HW_Base.consoleBlank('+',10); RT_HW_Base.consoleCurrentTime(); RT_HW_Base.consoleTest(String(F(" Подключение клиента cnt")),++ethServerCntConnect,'E','U');}
...
//-------------------------------------------------------------------------------------------------          
  ...
  id.stopConnection();
  if(mode>=1){RT_HW_Base.consoleBlank('-',10); RT_HW_Base.consoleCurrentTime(); RT_HW_Base.consoleTest(String(F(" Отключение  клиента cnt")),++ethServerCntDisConnect,'~','U');
              RT_HW_Base.consoleTest(String(F(" time(mcs)")),RT_HW_Base.getPastMcs(ethServerTimeBegin),'E','U');}
}
