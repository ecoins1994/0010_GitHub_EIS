...
RT_HW_STRUCT_NEXT_WRITE_VAR mIP_10;         //--IP Ethernet;
RT_HW_STRUCT_NEXT_WRITE_VAR mMAC1_11;       //--MAC-адрес Ethernet(байты 5-3);
RT_HW_STRUCT_NEXT_WRITE_VAR mMAC2_12;       //--MAC-адрес Ethernet(байты 2-0);
//--------------------------------------------------------------------------------------
...
if(mID_0.run>0)       {mID_0.var=RT_HW_Base.device.board.id;}                                     RT_HW_Next.sendINXX  (mID_0);
//-------------------------------4.1.Хар-ки платы---------------------------------------
...
if(mMAC1_11.run>0)    {mMAC1_11.var=(RT_HW_Base.device.board.mac[0]<<16) | (RT_HW_Base.device.board.mac[1]<<8) | (RT_HW_Base.device.board.mac[2]);}  RT_HW_Next.sendINXX(mMAC1_11);
//-------------------------------4.12.MAC2----------------------------------------------
if(mMAC2_12.custom==0){RT_HW_Next.setVarID(mMAC2_12, nxtNum,'A',500,2500); mMAC2_12.numReg=12;}   RT_HW_Next.directTask(mMAC2_12);
if(mMAC2_12.run>0)    {mMAC2_12.var=(RT_HW_Base.device.board.mac[3]<<16) | (RT_HW_Base.device.board.mac[4]<<8) | (RT_HW_Base.device.board.mac[5]);}  RT_HW_Next.sendINXX(mMAC2_12);
//-------------------------------4.23.DAC----------------------------------------------
