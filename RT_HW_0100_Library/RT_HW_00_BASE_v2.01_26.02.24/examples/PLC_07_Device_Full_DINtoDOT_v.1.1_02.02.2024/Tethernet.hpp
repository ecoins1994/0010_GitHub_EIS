...
//-------------------------------------------------------------------------------------------------
  void directNtpEth(){                              //==2.2.Управление NTP сервером
    ...
    if(ethNTP.fresh){ethNTP.fresh=0; RT_HW_Base.unixID.timeUNIX=RT_HW_Base.unixID.timeUNIX_NTP=ethNTP.timeUNIX;}}  //--Обновление времени от NTP сервера;
//-------------------------------------------------------------------------------------------------
  ...
  uint32_t getTimeUnixNTP(){return ethNTP.timeUNIX;}//==2.5.Получение времени timeUNIX от NTP;
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
#else
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
//-------------------------------------------------------------------------------------------------
           id.print("A0=");           id.print(vADS1115_A0f);        blankServerEth(id,'_',10);
           id.print("A1=");           id.print(vADS1115_A1f);         id.println("<br />");
//-------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------          
  ...
  ethServerTimePacket=RT_HW_Base.getPastMcs(ethServerTimeBegin);   
   if(mode>=1){RT_HW_Base.consoleBlank('-',9); RT_HW_Base.consoleCurrentTime(); 
               ...
               RT_HW_Base.ethernetIPtoMess(ethIP[0],ethIP[1],ethIP[2],ethIP[3]);
               RT_HW_Base.consoleTest(String(F("IP:")),RT_HW_Base.mess.str,'L');
