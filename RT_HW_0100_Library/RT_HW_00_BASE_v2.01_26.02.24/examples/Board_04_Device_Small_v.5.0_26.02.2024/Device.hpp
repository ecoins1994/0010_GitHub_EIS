...
RT_HW_GENERATOR_EVENT_ID    id74HC595_gen;  RT_HW_COUNTER_ID       id74HC595_cnt;   uint8_t v74HC595_cnt=0;    uint8_t v74HC595_gen=0, v74HC595_ago;  
...
void gen74HC595(){                                                              //==Функция: Генератор для 74HC595;
  ...
  if(RT_HW_Base.generatorEvent(id74HC595_gen,100)) {v74HC595_cnt=RT_HW_Base.counterS(id74HC595_cnt);  v74HC595=v74HC595_gen=RT_HW_Base.dc8(v74HC595_cnt);}}; //--Пилa, бегущая "1", 32 разряда;
//--------------------------------------------------------------------------------------------------
...
if(dv74HC595.custom==0){                                                        //--Настройка параметров устройства и задачи;
   ...
   RT_HW_74hc595.direct(dv74HC595,v74HC595,sh74HC595.run);};                    //--Управление устройством; 
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
#else
