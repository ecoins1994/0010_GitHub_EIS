...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
//-------------------------------------------------------------------------------------------------
//                                    1.0. Подключение библиотеки, создание объектов
//-------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------
//                                    1.1. Настройка параметров Ethernet
//-------------------------------------------------------------------------------------------------
  void setParamExpEth()
  {
    if(ethExp.custom==0)                             
    {                      
     eth.mac(macAddr0[0], macAddr0[1], macAddr0[2], macAddr1[0], macAddr1[1], macAddr1[2]);    
   ...
   //eth.setSpiBus(0);                              //--Здесь можно изменить номер шины SPI(default=RT_HW_Base.device.spi.busETH);    
     ethExp.custom=1;                               //--Установка состояние "Настройка выполнена успешно";
    }          
  };
//-------------------------------------------------------------------------------------------------
//                                    1.2. Управление Ethernet
//-------------------------------------------------------------------------------------------------
  void directExpEth()
  {
    ethExp.directExpEth(eth);   
    if(ethExp.freshStatus){
      ethIP0=eth.localIP();                       //--Сохранение IP адреса в формате IP;
      ethNumIP0=RT_HW_Base.ethIpToNumber(ethIP0); //--Сохранение IP адреса в формате uint32_t;
    }   //--Сохранение IP адреса;
  }; 
//-------------------------------------------------------------------------------------------------
//                                    1.3. Вывод сообщений Ethernet на консоль
//                                 (для диагностирования и тестирования оборудования)
//-------------------------------------------------------------------------------------------------
  void messExpEth(uint8_t vPeriod=0)
  {(void)vPeriod;
    ethExp.messExpEth(eth,vPeriod);
  };
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
#else
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  void messExpEth()    {;};   //==1.3.Вывод диагностических сообщений (для тестирования оборудования);
#endif
...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  
//-------------------------------------------------------------------------------------------------
//                                    2.0. Подключение библиотеки, создание объектов
//-------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------
//                                    2.1. Настройка параметров NTP-сервера
//-------------------------------------------------------------------------------------------------
  void setParamNtpEth()
  {
    if(ethNTP.custom==0)
...
//-------------------------------------------------------------------------------------------------
  ...
  { 
    ...
    if(ethNTP.fresh)
    {
       ethNTP.fresh=0; RT_HW_Base.unixID.timeUNIX=RT_HW_Base.unixID.timeUNIX_NTP=ethNTP.timeUNIX;
    }
  };  //--Обновление времени от NTP сервера;
//-------------------------------------------------------------------------------------------------
//                                    2.3. Вывод сообщений NTP-сервера
//-------------------------------------------------------------------------------------------------
  void messNtpEth()
  {
    ethNTP.messNtpEth();
...
//-------------------------------------------------------------------------------------------------
//                                    2.4. Получение времени от NTP-сервера
//-------------------------------------------------------------------------------------------------
  uint32_t getTimeUnixNTP()
  {
    return ethNTP.timeUNIX;
  };
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
#else
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  void setParamNtpEth() {;};                        //==2.1.Установка параметров NTP;
  void directNtpEth()   {;};                        //==2.2.Управление NTP сервером;
  void checkDirNtpEth() {;};                        //==2.3.Вывод номера текущего этапа(для тестирования программного обеспечения);
  void messNtpEth(){;};                              //==2.4.Вывод диагностических сообщений (для тестирования оборудования);
...
//
//-------------------------------------------------------------------------------------------------
           id.print("Core0=");        id.print(vSpeedMCUF0);   blankServerEth(id,'_',10);
           id.print("Core1=");        id.print(vSpeedMCUF1);   blankServerEth(id,'_',10);   
           id.print("MotorSec=");     id.print(RT_HW_Base.shed.cntSec);    id.println("<br />");
//-------------------------------------------------------------------------------------------------
           id.print("ADCF=");         id.print(vADCF);         blankServerEth(id,'_',10);
           id.print("A1_ADS115=");    id.print(vADS1115_A1f);  id.println("<br />");           
//-------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------          
  ...
  ethServerTimePacket=RT_HW_Base.getPastMcs(ethServerTimeBegin);   
   if(mode>=1){RT_HW_Base.consoleBlank('-',9); RT_HW_Base.consoleCurrentTime(); 
               ...
               RT_HW_Base.consoleVar(ethIP0[0],'.',0,'U');
               RT_HW_Base.consoleVar(ethIP0[1],'.',0,'U');
               RT_HW_Base.consoleVar(ethIP0[2],'.',0,'U');
               RT_HW_Base.consoleVar(ethIP0[3],'~',0,'U');
...
//-------------------------------------------------------------------------------------------------
#else
void directServerEth(uint8_t mode){(void)mode;};
void messServerEth()  {;};
#endif
//=================================================================================================
