...
//------------------------------------------------------------------------------------------------- 
  ...
  uint32_t timeUNIX_NTP;    //--Время от NTP-сервера;
  uint16_t vDeltaNTP;       //--Разница с текущим временем от  NTP-сервера(& 0xFFFF);
//-------------------------------------------------------------------------------------------------
...
void fncInitParam(){
  if(RT_HW_Base.isInit==1){return;}
     ...
     ethNumIP0=((uint32_t)192<<24) | ((uint32_t)168<<16) | ((uint32_t)2<<8) | 70;
...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  RT_HW_FILTER_RING_AVERAGE<uint16_t,5> filterADC;                      //--Фильтр ADC(median 5-го порядка), значение после фильтра;
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void fncADC(){
