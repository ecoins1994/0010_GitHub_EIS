...
id.ready=1; 
id.event=0; id.quick.num=id.fast.num=id.slow.num=id.back.num=id.frdm.num=0; id.eventSec=0;		//--Reset parameters;
                                                           ++id.cntCycle;   id.control^=1; 		//--Control loop();
//-------------------------------------------------------------------------------------------------
if(period(id.timeStart10,micros(),10000)) {id.run^=1;   id.event^=1;   id.blink10^=1;    		//--Quick task, blink10 10ms;
     if(++id.workSec>=100){id.workSec=0;  id.cntSec++;  id.eventSec=1; id.blinkSec^=1;	 		//--Working time (sec);
id.cycle=id.cntCycle; id.cntCycle=0;  id.smallCycle=(uint16_t)round(((float)id.cycle)/1000);}}	//--Project performance;
//-------------------------------------------------------------------------------------------------
...
else{id.event^=(1<<4);  if(++id.frdm.cnt  >id.frdm.qnt) {id.frdm.cnt =1;} id.frdm.num =id.frdm.cnt;                      return;}
};
